#' Generate Base Flow Index (BFI) map from soil (HOST) map
#'
...
#'
#' @return BFI number in the range [0,1] that represents the BFI calculated only using dominant soil classes
#'
#' @examples
#' # BFIHOSTdominant <- BFIHOST(soilMap=mySoilMap,shpFolder=myShpFolder,mask="mask_tmp")
#'

BFIHOSTdominant <- function(soilMap,shpFolder,mask,myCRS=NULL){
  ...
  rawMask <- readOGR(dsn=shpFolder,layer=mask,verbose=FALSE)
